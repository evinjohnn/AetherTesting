...
const AdminMission = () => {
  ...
  const fetchMission = async () => {
    ...
    try {
      ...
      setmission(response.data.data.missions);
      setIsLoading(false);
  ...
  const handleDelete = () => {
    deleteOperation("mission", storedId);
  ...
  const handleViewClick = (data) => {
    navigate("/admin/mission/detail", { state: { data } });
  };
    ...
    <Section>
      <div>
        {/* <Header>
          ...
          <AddButton onClick={openModal}>Add New Mission</AddButton>
        </Header> */}
        {mission && mission.length <= 2 ? (
          <AdminButton
            head="Manage Missions"
            button="Add New Mission and vision"
            display={true}
          />
        ) : (
          <AdminButton
            head="manage Missions"
            button="Add New Mission"
            onClick={openModal}
          />
        )}
        ...
        <MainContent>
          ...
          ) : mission.length === 0 ? ( // Check if data is empty
