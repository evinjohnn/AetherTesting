...
const Form = () => {
  ...
  const fetchContactSection = async () => {
    ...
    try {
      const response = await clientaxiosInstance.get(`/contact-section`);
      setIsLoading(false);
  ...
  // };
  //handle change, with phonenumber validation check( alphabets and special charachters)
  const handleChange = (e) => {
    ...
    if (name === "mobile") {
      if (!regex.test(value)) {
        setError("Invalid phone number. Only digits are allowed.");
        return;
      }
      if (value.length > 15) {
      ...
      <div className="wrapper">
        ...
        <ButtonContainer className="bottom">
          <Button>
            <Line className="top" />
            <Line className="right" />
            <ButtonComponent text="Ready To Get Started" />
            <Line className="bottom" />
            <Line className="left" />
          </Button>{" "}
        </ButtonContainer>
