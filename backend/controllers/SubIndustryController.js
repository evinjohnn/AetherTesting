...
const SubIndustry = require("../models/SubIndustry");
...
const IndustryController = {
  async create(req, res) {
    try {
      ...
      const industry = new SubIndustry({  name, description, route, image: imageUrl });
      await industry.save();
  ...
  async getAll(req, res) {
    try {
      ...
      if (isNeedFull) {
        const industrys = await SubIndustry.find();
        ...
        const totalIndustrys = await SubIndustry.countDocuments();
  ...
  async getById(req, res) {
    try {
      const industry = await SubIndustry.findById(req.params.id);
  ...
  async update(req, res) {
    try {
      ...
      if (req.file) {
        const findImage = await SubIndustry.findById(req.params.id);
      ...
      const industry = await SubIndustry.findByIdAndUpdate(
        req.params.id,
        updatePayload,
        {
          new: true,
  ...
  async delete(req, res) {
    try {
      const industry = await SubIndustry.findByIdAndDelete(req.params.id);
      if (!industry) return res.badResponse("industry not found");
